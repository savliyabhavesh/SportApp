                            ...
                            checked
                            {
                                ...
                                try
                                {
                                    if (toparty != "" && amount != 0 && toparty != txtname1.Text)
                                    {
                                        ...
                                        string cmdText3 = $"insert into Trans(tns_Party,tns_Amount,tns_Remark,tns_MatchID,tns_dt,tns_Type,tns_Vid,tns_Monday,tns_Sessionid,tns_ModifyID,tns_Monday_Final,tns_Hawala,tns_chk,tns_time,tns_date) values('{toparty}',{amt1},'{this.txtname1.Text + " (Auto)"}','0','{Strings.FormatDateTime(DateAndTime.Now, DateFormat.ShortDate)}','{(amt1 > 0 ? "CR" : "DR")}',0,0,0,{modifyid},'No',0,0,'{Strings.FormatDateTime(DateAndTime.TimeOfDay, DateFormat.LongTime)}','{Strings.FormatDateTime(DateAndTime.Now, DateFormat.ShortDate)}')";

                                        string cmdText4 = $"insert into Trans(tns_Party,tns_Amount,tns_Remark,tns_MatchID,tns_dt,tns_Type,tns_Vid,tns_Monday,tns_Sessionid,tns_ModifyID,tns_Monday_Final,tns_Hawala,tns_chk,tns_time,tns_date) values('{this.txtname1.Text}',{amount},'{toparty + " (Auto)"}','0','{Strings.FormatDateTime(DateAndTime.Now, DateFormat.ShortDate)}','{(amount > 0 ? "CR" : "DR")}', 0,0,0,{modifyid},'No','1',0,'{Strings.FormatDateTime(DateAndTime.TimeOfDay, DateFormat.LongTime)}','{Strings.FormatDateTime(DateAndTime.Now, DateFormat.ShortDate)}')";
