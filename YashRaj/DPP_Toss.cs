...
namespace YashAksh
{
    ...
    //[DesignerGenerated]
    public partial class DPP_Toss : Form
    {
        ...
        {
            MyProject.Forms.Toss_Match.Activate();
            this.Close();
        }

        // Token: 0x06000BEA RID: 3050 RVA: 0x002FD364 File Offset: 0x002FB764
        private void DPP_Toss_Activated(object sender, EventArgs e)
        {
            this.txtname.Focus();
        }

        // Token: 0x06000BEB RID: 3051 RVA: 0x002FD374 File Offset: 0x002FB774
        private void DPP_Toss_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
        ...
        private void DPP_Toss_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                this.testnu.Text = Conversions.ToString(Module1.tossmatchid);
                //this.YES_NO();				
            ...
            checked
            {
                try
                {
                    ...
                    if (Operators.CompareString(this.txtname.Text, null, false) != 0)
                    {
                        ...
                        {
                            "SELECT id, m_id, m_eid, m_page, m_amt, m_rate, m_team, m_mode, m_party, m_dt, m_time, m_team1, m_team2, m_team3, m_team4, Modify, m_checked, M_chk, modify_time, modify_date FROM MatchTrans1 WHERE (m_id = ",
                            this.testnu.Text,
                            ") AND (m_party = '",
                            this.txtname.Text,
                            "') ORDER BY m_eid DESC"
        ...
        public void nptotal()
        {
            try
            {
                ...
                {
                    "SELECT PartyMaster.PartyName, PartyMaster.MatchCommiType, PartyMaster.Status, MatchTrans1.m_id, MatchTrans1.m_checked, SUM(MatchTrans1.m_team1) AS Expr1, SUM(MatchTrans1.m_team2) AS Expr2, SUM(MatchTrans1.m_team3) AS Expr3, SUM(MatchTrans1.m_team4)  AS Expr4, PartyMaster.SMatch FROM (MatchTrans1 INNER JOIN PartyMaster ON MatchTrans1.m_party = PartyMaster.PartyName) GROUP BY PartyMaster.PartyName, PartyMaster.MatchCommiType, PartyMaster.Status, MatchTrans1.m_id, MatchTrans1.m_checked, PartyMaster.SMatch HAVING (PartyMaster.PartyName = '",
                    this.txtname.Text,
                    "') AND (MatchTrans1.m_id = ",
                    this.testnu.Text,
                    ") AND (MatchTrans1.m_checked = 0) ORDER BY PartyMaster.PartyName"
                });
                OleDbCommand oleDbCommand = new OleDbCommand(cmdText, Module1.conn);
                OleDbDataReader oleDbDataReader = oleDbCommand.ExecuteReader();
        ...
        private void Button4_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                reportDocument.Load(Application.StartupPath + "\\RPT\\TossDPP.rpt");
        ...
        public void Savedata_all()
        {
            try
            {
                string cmdText = $"UPDATE MatchTrans1 SET m_page = '{this.txtpage.Text}', m_amt = {this.txtamount.Text}, m_rate = '{this.txtrate.Text}', m_team = '{this.cmbteam.Text}', m_mode = '{this.ComboBox2.Text}', m_party = '{this.txtpartiname.Text}', Status = 'N', modify_date = '{Conversions.ToString(DateAndTime.Now.Date)}', modify_time = '{Strings.FormatDateTime(DateAndTime.TimeOfDay, DateFormat.LongTime)}', m_team1 = {this.txtpak.Text}, m_team2 = {this.txtend.Text}, m_team3 ={this.txtsut.Text}, m_team4 = {this.txtind.Text}, Modify = '1', m_checked = 0, M_P = '0', M_chk = 0 WHERE MatchTrans1.m_id = {this.testnu.Text} AND MatchTrans1.m_eid = {this.txtmodify.Text}";
            ...
            try
            {
                if (Interaction.MsgBox("Are you sure want to delete?", MsgBoxStyle.YesNo, null) == MsgBoxResult.Yes)
                {
                    if (Operators.CompareString(this.txtmodify.Text, null, false) != 0)
                    {
                        ...
                        {
                            Module1.conn.Close();
                            string cmdText2 = $"Update MatchTrans1 Set m_checked=1,m_chk=0, modify_date = '{Conversions.ToString(DateAndTime.Now.Date)}', modify_time = '{ Strings.FormatDateTime(DateAndTime.TimeOfDay, DateFormat.LongTime) }' where m_id={ this.testnu.Text } and m_eid={ this.txtmodify.Text}";
                            using (MatchService matchService = new MatchService())
                            {
                    ...
                    for (int i = 0; i <= this.DataGridView1.Rows.Count - 1; i++)
                    {
                        unchecked
                        {
                            if (Operators.ConditionalCompareObjectEqual(this.DataGridView1.Rows[i].Cells["M_Checked"].Value, "0", false))
                            {
                                ...
                                num5 += Convert.ToDouble(RuntimeHelpers.GetObjectValue(this.DataGridView1.Rows[i].Cells[10].Value));
                                num6 += Convert.ToDouble(RuntimeHelpers.GetObjectValue(this.DataGridView1.Rows[i].Cells[11].Value));
                                this.Strsql = $"UPDATE MatchTrans1 SET M_chk = 1 WHERE m_checked=0 AND m_id = { this.testnu.Text } AND m_eid = { this.DataGridView1.Rows[i].Cells[1].Value}";
                                using (MatchService matchService = new MatchService())
                                {
            ...
            checked
            {
                try
                {
                    if (e.ColumnIndex == 0)
                    {
                        ...
                        {
                            this.Strsql = $"UPDATE MatchTrans1 SET M_chk = 1 WHERE m_checked=0 AND m_id = { this.testnu.Text } AND m_eid = { this.DataGridView1.CurrentRow.Cells[1].Value}";
                            using (MatchService matchService = new MatchService())
                            {
                                matchService.Update(Strsql);
                            }
                            ...
                            && Operators.ConditionalCompareObjectEqual(this.DataGridView1.CurrentRow.Cells["M_Checked"].Value, "0", false))
                        {
                            this.Strsql = $"UPDATE MatchTrans1 SET M_chk = 0 WHERE m_checked=0 AND m_id = { this.testnu.Text } AND m_eid = { this.DataGridView1.CurrentRow.Cells[1].Value}";
                            using (MatchService matchService = new MatchService())
                            {
                                matchService.Update(Strsql);
                            }
